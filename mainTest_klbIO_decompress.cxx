...
{
	std::string filename;//("/home/trihuynh/Desktop/Research/softwares/Amat/Data/SPM00_TM000001_CM01_CHN01.corrected.klb");
	ofstream outfilehdr;
	ofstream outfileraw;
	if (argc>=3)
	{
		...
		char tmpname[200];
		sprintf(tmpname,"%s.hdr",argv[2]);
		outfilehdr.open(tmpname);
		sprintf(tmpname,"%s.raw",argv[2]);
		outfileraw.open(tmpname,ios::out|ios::binary);
	}
	else
	{		
		return 0;
...
*/	
	...
	for (int i=0; i<5; i++)
	{
		printf("Dim xyzct[%d]=%d\n",i,imgFull.header.xyzct[i]);
		printf("pixelSize[%d]=%f\n",i,imgFull.header.pixelSize[i]);
		
	}
	*/
	outfilehdr << "Dimension: ";
	for (int i=0; i<5; i++)
		outfilehdr << imgFull.header.xyzct[i] <<" ";
	outfilehdr << endl;
	outfilehdr << "Pixel Size: ";
	for (int i=0; i<5; i++)
		outfilehdr << imgFull.header.pixelSize[i] <<" ";
	outfilehdr.close();
	cout<<"Image header written\n";
	...
	t2 = Clock::now();
	outfileraw.write((char*)imgA,N*sizeof(uint16_t));
	outfileraw.close();
	cout<<"Image raw content written\n";
	std::cout << "Read full test file at " << filename << " in =" << std::chrono::duration_cast<std::chrono::milliseconds>(t2 - t1).count() << " ms using " << numThreads << " threads" << std::endl;
...
*/	
	...
	if (err > 0)
		return err;
	dim = 2;
	planeSize = imgFull.header.xyzct[0] * imgFull.header.xyzct[1];
	offsetPlane = 1;
	...
	totalTime = 0; 
	numPlanes = numPlanes = std::min(imgFull.header.xyzct[dim], 100U);
	for (uint64_t ii = 0; ii < numPlanes; ii++)
	{
		t1 = Clock::now();
		ROIfull.defineSlice(ii, dim, imgFull.header.xyzct);
		err = imgFull.readImage((char*)imgB, &ROIfull, numThreads);
		if (err > 0)
			return err;
		t2 = Clock::now();
	...
	std::cout << "Read all planes test file at " << filename << " in =" <<  ((float)totalTime)/ (float)numPlanes << " ms per plane using " << numThreads << " threads" << std::endl;
	delete[] imgB;
